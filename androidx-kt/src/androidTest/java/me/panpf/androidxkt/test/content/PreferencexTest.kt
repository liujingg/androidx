...
class PreferencexTest {
    ...
    fun testGetPreferences() {
        ...
        Assert.assertEquals("testGetPreferences.xml", (preferences.getFieldValue("mFile") as File?).requireNotNull().name)

        val defaultPreferences = context.getDefaultSharedPreferences()
        Assert.assertEquals(context.packageName + "_preferences.xml", (defaultPreferences.getFieldValue("mFile") as File?).requireNotNull().name)
    }
