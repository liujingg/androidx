...
dependencies {
    ...
    api("androidx.lifecycle:lifecycle-viewmodel:${property("ANDROIDX_LIFECYCLE")}")
    api("androidx.lifecycle:lifecycle-extensions:${property("ANDROIDX_LIFECYCLE")}")
...
Properties().apply { project.file("local.properties").takeIf { it.exists() }?.inputStream()?.use { load(it) } }.takeIf { !it.isEmpty }?.let { localProperties ->
    ...
    configure<PublishExtension> {
        groupId = "me.panpf"
        artifactId = "androidx-kt-arch"
        publishVersion = property("VERSION_NAME").toString()
        desc = "Adnroid, Tool, Kotlin, Arch"
